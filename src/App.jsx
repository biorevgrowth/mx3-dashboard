...
function useData() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const [snapshot, company_goal, verticals, regions, history, briefing] = await Promise.all([
      ...
      <div style={{ marginLeft: 56, padding: "28px 36px", maxWidth: 1100 }}>
        ...
        {view === "briefing" && (
          <>
            {data.briefing ? (
              <>
                <Section title={`Daily Intelligence Brief — ${new Date(data.briefing.briefing_date).toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric", year: "numeric" })}`}>
                  <div style={{ fontSize: 14, lineHeight: 1.7, color: T.text, whiteSpace: "pre-wrap" }}>
                    {data.briefing.summary}
                  </div>
                </Section>

                <Section title="EOY Projection">
                  <div style={{ fontSize: 14, lineHeight: 1.7, color: T.text, whiteSpace: "pre-wrap" }}>
                    {data.briefing.projection}
                  </div>
                </Section>

                <Section title="Recommended Actions">
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {(data.briefing.actions || []).map((action, i) => (
                      <div key={i} style={{
                        background: T.surfaceAlt, borderRadius: 8, padding: "14px 16px",
                        borderLeft: `3px solid ${
                          action.urgency === "high" ? T.red :
                          action.urgency === "medium" ? T.amber : T.green
                        }`,
                      }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                          <span style={{ fontSize: 12, fontWeight: 600, color: T.text }}>{action.signal}</span>
                          <span style={{
                            fontSize: 10, fontWeight: 600, textTransform: "uppercase",
                            color: action.urgency === "high" ? T.red : action.urgency === "medium" ? T.amber : T.green,
                            background: action.urgency === "high" ? T.redGlow : action.urgency === "medium" ? T.amberGlow : T.greenGlow,
                            padding: "2px 8px", borderRadius: 4,
                          }}>{action.urgency}</span>
                        </div>
                        <p style={{ fontSize: 13, color: T.textSec, margin: 0, lineHeight: 1.5 }}>{action.action}</p>
                      </div>
                    ))}
                  </div>
                </Section>
                ...
                <div style={{ fontSize: 11, color: T.textMute, marginTop: 8 }}>
                  Generated by {data.briefing.model_used || "AI"} at {new Date(data.briefing.created_at).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}
                </div>
              </>
            ) : (
              <Section title="Daily Intelligence Brief">
                <div style={{
                  textAlign: "center", padding: "40px 0", color: T.textMute,
                }}>
                  <p style={{ fontSize: 32, margin: "0 0 12px" }}>{"\u2691"}</p>
                  <p style={{ fontSize: 14, margin: "0 0 6px", color: T.textSec }}>No briefing available yet</p>
                  <p style={{ fontSize: 12, margin: 0 }}>
                    The daily intelligence brief will appear here once the n8n pipeline generates its first report at 6:00 AM CST.
                  </p>
                </div>
              </Section>
            )}
